...
export const DEFAULT_BINDINGS = {
    ...
    down:  ['KeyS', 'ArrowDown'],
    shoot: ['Numpad4', 'KeyJ'],
    jump:  ['Numpad5', 'Space', 'KeyK'],
    dash:  ['Numpad6', 'ShiftLeft', 'KeyL'],
    start: ['Enter'],
    select: ['Backspace'],
};
...
const ANALOG_X     = 0;
const ANALOG_Y     = 1;
const ANALOG_DEAD  = 0.3;  // deadzone — ignore stick drift below this

export class Input {
    constructor(bindings = DEFAULT_BINDINGS) {
        // Deep copy so DEFAULT_BINDINGS stays pristine for resets
        this.bindings = {};
        for (const action of Object.keys(bindings)) {
            this.bindings[action] = [...bindings[action]];
        }
        this.rawKeys = {};
        this.current = {};
        this.prev = {};
        ...
        // Raw key capture for rebinding UI
        this.lastRawKey = null;

        window.addEventListener('keydown', (e) => {
            this.rawKeys[e.code] = true;
            this.lastRawKey = e.code;
    ...
    /** Read and clear the last raw keydown code (for rebinding UI). */
    consumeRawKey() {
        const key = this.lastRawKey;
        this.lastRawKey = null;
        return key;
    }
}
